const http=require("http"),fs=require("fs"),path=require("path"),url=require("url"),QRCode=require("qrcode");
...
const server=http.createServer(async(req,res)=>{
 ...
 if(req.method==="POST"&&p==="/api/login"){let b=await parseBody(req);if(String(b.password)===ADMIN_PASSWORD){let sid=secrets.tokenHex(20);sessions.add(sid);res.writeHead(200,{"Set-Cookie":`sid=${sid}; HttpOnly; SameSite=Lax; Path=/`,"Content-Type":"application/json"});return res.end('{"ok":true}')}return json(res,401,{error:"密码错误"})}
